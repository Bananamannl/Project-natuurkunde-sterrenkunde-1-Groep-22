...
parameters = parameters_timeseries(Q1, Q2, window_size=500, step_size=20)
parameters = np.repeat(parameters, repeats=20, axis=0)
...
def orthagonal_displacement_and_parameter(HoQIs, matrix, a):
    ...
    """
    N = min(len(HoQIs), len(a))
    vectors = HoQIs[:N] @ matrix.T
    a = a[:N]

HoQI1x_a = orthagonal_displacement_and_parameter(HoQIs, matrix_1x, a)

cloud = pv.PolyData(HoQI1x_a)
...
max_range = np.max(np.abs(HoQI1x_a))
