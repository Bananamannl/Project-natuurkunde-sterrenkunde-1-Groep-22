...
def standard_step_window_ellipse_fitting(Q1, Q2, window_size):
    ...
    return np.array(return_Q1), np.array(return_Q2)
...
def variable_step_window_ellipse_fitting(Q1, Q2, window_size, step_size):
    ...
    for start in range(window_size-step_size, len(Q1) - window_size + 1, step_size):
        ...
        list_x0, list_y0, list_a, list_b, list_theta = np.transpose(params_matrix[int((start / step_size) - (window_size / step_size) + 1) : int((start / step_size) + (window_size / step_size) - 1), : ])
        vectors = np.column_stack((part_Q1, part_Q2))
...
for h in range(0,6):
    if h <= 2:
        fitted_Q1, fitted_Q2 = variable_step_window_ellipse_fitting(Q1_list[h], Q2_list[h], window_size=1000, step_size=1)
    else:
        fitted_Q1, fitted_Q2 = variable_step_window_ellipse_fitting(Q1_list[h], Q2_list[h], window_size=100, step_size=1)
